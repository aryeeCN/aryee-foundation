...
import cn.aryee.gateway.reactive.autoconfigure.GatewayReactiveProperties;
...
/**
 * Gateway Reactive 集成测试
 * 测试响应式网关功能：限流、熔断、灰度发布、API 聚合
 *
 * @author Aryee
 * @since 1.0.0
 */
@SpringBootTest
@DisplayName("Gateway Reactive 集成测试")
@RequiredArgsConstructor
@TestConstructor(autowireMode = TestConstructor.AutowireMode.ALL)
public class GatewayReactiveIntegrationTest {

    private final GatewayReactiveProperties gatewayProperties;
    ...
    @Test
    @DisplayName("Reactive Gateway 配置加载测试")
    void testGatewayPropertiesLoaded() {
        assertNotNull(gatewayProperties, "GatewayReactiveProperties 应该被加载");
        assertTrue(gatewayProperties.isEnabled(), "Gateway 模块应该启用");
    }

    @Test
    @DisplayName("Reactive 限流器获取许可测试")
    void testRateLimiterAcquire() {
        String key = "reactive-test-" + System.currentTimeMillis();
    ...
    @Test
    @DisplayName("Reactive 限流器剩余许可查询测试")
    void testRateLimiterRemaining() {
        String key = "reactive-remaining-" + System.currentTimeMillis();
    ...
    @Test
    @DisplayName("Reactive 限流器重置测试")
    void testRateLimiterReset() {
        String key = "reactive-reset-" + System.currentTimeMillis();
        rateLimiter.tryAcquire(key);
    ...
    @Test
    @DisplayName("Reactive 熔断器初始状态测试")
    ...
    @Test
    @DisplayName("Reactive 熔断器执行成功操作测试")
    ...
    @Test
    @DisplayName("Reactive 灰度用户管理测试")
    void testCanaryUserManagement() {
        String testUser = "reactive-canary-user-" + System.currentTimeMillis();
    ...
    @Test
    @DisplayName("Reactive API 聚合器配置测试")
